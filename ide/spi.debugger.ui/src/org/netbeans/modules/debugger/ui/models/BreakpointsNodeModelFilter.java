...
@DebuggerServiceRegistration(path="BreakpointsView",
                             ...
                             position=10100)
public class BreakpointsNodeModelFilter implements ExtendedNodeModelFilter {
    ...
    public String getDisplayName(NodeModel original, Object node) throws UnknownTypeException {
        if (node instanceof Breakpoint) {
            String description = ((Breakpoint) node).getDescription();
            if (!description.equals("")) {
                return original.getDisplayName(node) + " (" + ((Breakpoint) node).getDescription() + ')';
            }
        }
